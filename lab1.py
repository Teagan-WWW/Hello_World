...
#
def opposite(S):
    return(int(runway(S)+180)%360)
#post-lab Teacher Solution:
# return(runway((S)+180)-1)%360+1)
#note, original solution 90% there
#subtracting then adding back 1 circumvents the issue of 180 returning 0.
#start with something close to the solution, see the issues, retry
#don't get trapped in a possible solution if it's not gonna work.
...
def countTables(n, s):
    return((n+(s-1))//(s))
#Teacher Solution:
#start with (n//s). However, leaves remainder guests unseated.
#Shift the range, similar to opposite(s), by adding (s-1) to n.
#changing 110//10 to 109//10 will result in the same amount of tables.
#Teacher solution same as original.
...
#Teacher Solution:
#(h+tz)%24 gives us the local hour in 24hourtime, turn it into a string,
#turn ':' into a string, turn 'm' into a string.
#Add all strings together.
#Teacher solution same as orginal.
